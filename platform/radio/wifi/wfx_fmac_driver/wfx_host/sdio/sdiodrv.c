                                            ...
                                            uint8_t *data)
{
  SDIO_Cmd_t cmd53 = {
    ...
    .dataDirection = SDIO_DATA_DIRECTION_CARD_TO_HOST, // Set Data Transfer Direction
    .respType = SDIO_RESP_TYPE_RESP48,
    .checkCrc = true,
    .checkIndex = true,
    .enableDma = true,
  ...
  if (op == SDIODRV_IO_OP_WRITE) {
    cmd53.args |= (uint32_t)(!!op << CMD53_RW_FLAG_SHIFT);
    cmd53.dataDirection = SDIO_DATA_DIRECTION_HOST_TO_CARD; // Set Data Transfer Direction
  }
...
void SDIO_IRQHandler (void)
{
  ...
  if ((pending & SDIO_IFCR_DMAINT) == SDIO_IFCR_DMAINT) {
    SDIO->SDMASYSADDR = SDIO->SDMASYSADDR; // DMA Interrupt
  }
